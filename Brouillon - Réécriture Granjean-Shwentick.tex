...
\begin{document}
	...
	\begin{remark}
		...
		La sortie du LSRS peut aussi être vue comme une nouvelle structure $s'$ de type $\{f_1, \dots, f_k\}$.
	\end{remark}
	
	
	\begin{definition}[RAM $n^k$-représentée]
		Soient $t_1, t_2$ des types. Soit $\Gamma$ une $(t_1, t_2)$-fonction de RAM. 
		
		Soit $S$ un LSRS pour $f_1, \dots, f_k$ sur $F_{t_1}$. On dit que $\Gamma$ est $n^k$-représentée par $S$ lorsqu'il existe un entier $c$ et une projection affine $P$ tels que, pour chaque structure $s$ $c$-bornée, $S$ définit des fonctions $f_1, \dots, f_k : c (s.n)^k \to c (s.n)^k$ à partir des interprétations des fonctions de $F_{t_1}$ suivantes :
		
		
		Ce LSRS définit une structure $s'$ de type $\{f_1, \dots, f_k\}$. 
	\end{definition}
